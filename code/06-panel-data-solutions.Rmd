...
```{r}
data("kielmc")
summary(model_before <- lm(rprice ~ nearinc, data = filter(kielmc, year == 1978)))
summary(model_after <- lm(rprice ~ nearinc, data = filter(kielmc, year == 1981)))
# the implied net effect of incinerator installment
coef(model_after)[2] - coef(model_before)[2]
```
...
```{r}
summary(model_after <- lm(rprice ~ nearinc*y81, data = kielmc))
```
...
```{r}
summary(model_after <- lm(rprice ~ nearinc*y81 + age + intst + land + area + rooms + baths, data = kielmc))
```
...
```{r}
summary(lm(trmgpa ~ sat + female + season, data = gpa3))
```
...
```{r}
summary(plm(trmgpa ~ sat + female + season, data = gpa3, model = "fd", index = "id"))
summary(plm(trmgpa ~ sat + female + season, data = gpa3, model = "within", index = "id"))
```
...
```{r}
data(wagepan)
summary(plm(lwage ~ educ + black + hisp + exper, data = wagepan, index = c("nr", "year"), model = "pooling"))
```
...
```{r}
summary(model_fe <- plm(lwage ~ educ + black + hisp + exper, data = wagepan, index = c("nr", "year"), model = "within"))
summary(model_re <- plm(lwage ~ educ + black + hisp + exper, data = wagepan, index = c("nr", "year"), model = "random"))
```
